...
@Injectable()
export class ProviderExportService implements IExportService {
	exportExcel(products: Producto[], provider: string): any {
		var toExport: any = [];
		products.forEach(product => {
			var esPromo = 'no';
			if (product.esPromo == true) {
				esPromo = 'si';
			}
			if (product.categoria != null) {
				product.linea = product.categoria.nombre;
				if (product.categoria.padre != null) {
					product.division = product.categoria.padre.nombre;
				}
			}
			...
			var productToExport = {
				...
				'Ancho Pallet': anchoPallet,
				'Profundidad Pallet': profundidadPallet,
